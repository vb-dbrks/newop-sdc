...
def require_member(study_id: UUID, user: CurrentUserDep) -> CurrentUser:
    """Caller must hold any role on this study; else 404 (don't leak existence)."""
    # TODO: check study_role_assignments
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)


def require_author(study_id: UUID, user: CurrentUserDep) -> CurrentUser:
    """Caller must be Author on this study."""
    # TODO: check study_role_assignments role='author'
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)


def require_reviewer(study_id: UUID, user: CurrentUserDep) -> CurrentUser:
